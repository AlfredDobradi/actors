...
func (a *AccountActor) replayTicks(ctx context.Context, since int64) {
	spanID := telemetry.SpanIDFromContext(ctx)
	...
	// TODO Add better error handling: if there's an issue, stop replaying and persist the last tick replayed as timestamp
	for range ticksSinceTime {
		if err := a.processTick(ctx, &system.Message{}); err != nil {
			ctxLogger.Error("Error processing tick during replay", "actor_id", a.GetID(), "error", err)
		}
	}
